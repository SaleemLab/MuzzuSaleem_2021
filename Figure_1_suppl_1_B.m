%% Tomaso Muzzu - UCL - 19/03/2020 - Figure 1 suppl 1B

%% Functions to plot Figure 1 suppl 1 - direction tuning
...
Rec_OI = unique(AM_ParamSel(1,UnitsPertPos,1)); % recordings of interest
...
for i = 1:size(SpeedResponse_nSel,2)
    ...
    DiffResponsePercent_Pert(:,i,6) = (nanmean(UnitResponse(:,po_i(1):po_i(2)-trialSide_samples/2),2)-nanmean(UnitResponse(:,po_i(1)-trialSide_samples:po_i(1)),2))./...
                                      (nanmean(UnitResponse(:,po_i(1)-trialSide_samples:po_i(1)),2)); % (mean(pert)-mean(prepert))/mean(prepert) a.k.a modulation index    
end
...
parfor i = 3:size(DiffResponsePercent_Pert,2)
    SingleUnitResponse = squeeze(DiffResponsePercent_Pert(:,i,:));
    % select trial responses
    Responses = Param(:,i);
    % scrub responses
    ...
    for sh_i = 1:1000
        ...
        % step 3: compute the ROC and evaluate the AUC for each unit
        [X,Y,T,AUC1] = perfcurve(Responses,scores,1);
        AUC_shuffled_Sp_(i,sh_i) = AUC1;
    end
    i
end
toc
% 
AUC_shuffled_Sp = cat(2,AUC_speed(:,1),AUC_shuffled_Sp_);
save('AUC_shuffled_Speed_ALL.mat','AUC_shuffled_Sp')
...
% AUC_speed_n = AUC_shuffled_Sp(:,1);
% AUC_speed_shuffle_= AUC_shuffled_Sp(:,2:end);
% p_sh = prctile(AUC_speed_shuffle_(:),99);
% find(AUC_speed>p_sh)
...
figure
h = histogram(AUC_lin(:),[0:0.02:1],'FaceColor',[0.5 0.5 0.5],'Normalization','probability','EdgeColor','none')
hold on
...
%% Count units recorded from recordings where speed changes significantly
ROI_sp = find(AUC_2_plot(:,1)>p_sh);
ROI_sp_i = RecIndex(ROI_sp);
clear ROI_sp_units Tot_Units
for i = 1:length(ROI_sp_i)
    ROI_sp_units(i) = sum(UnitsPertPos(find(AM_ParamSel(1,:,1)==ROI_sp_i(i))));
    Tot_Units(i) = length(find(AM_ParamSel(1,:,1)==ROI_sp_i(i)));
end
% fraction of pert resp units 
sum(ROI_sp_units)/sum(Tot_Units)

%% count units recorded from recordings where speed does NOT change significantly
ROI_sp = find(AUC_2_plot(:,1)<p_sh);
ROI_sp_i = RecIndex(ROI_sp);
clear ROI_sp_units Tot_Units
for i = 1:length(ROI_sp_i)
...
AUC_2_plot(1,1)
prctile(AUC_2_plot(2,2:end),99)

i = 2
AUC_speed(i)
for i = 1:size(AUC_2_plot,1)
    P_th_i(i) = prctile(AUC_2_plot(i,2:end),99)
end

AUC_speed
AUC_2_plot(:,1)

prctile(AUC_speed_shuffle_(i,:),99)
...
%%
RespUnits_idx=find(UnitsPertPos);
for i = 1:length(RecIndex)
...
RespUnits_idx=find(UnitsPertPos); k = 1; clear rho rho MI_corr MI_speed MI_neuro Speed_trial
for i = 1:length(RecIndex)
    Unit_RoI = find(AM_ParamSel(1,UnitsPertPos,1)==RecIndex(i));
    Unit_idx = RespUnits_idx(Unit_RoI);
    PertTrials_1 = AM_ParamSel(:,RecIndex(i),3)==1;
    PertTrials_0 = AM_ParamSel(:,RecIndex(i),3)==0;
    for j = 1:length(Unit_idx)
...
ylabel('distribution of median R'); xlabel('Pearson correlation R')
