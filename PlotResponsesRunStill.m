...
function PlotResponsesRunStill(ProjectData,SelectedResponses,AM_Param,AM_Speed,Units_Sel,Diff,Sep)
...
if Diff==0
    if Sep==0
        ...
        imagescSetting(TimeLine);
    else
        subplot(10,2,[9:2:19])
        imagesc(TimeLine,1:size(SelectedResponses{1}(I,:),1),SelectedResponses{1}(I,:));
        imagescSetting(TimeLine);
        %control
        subplot(10,2,[10:2:20])
        [B,I,PertTrial_Ex] = PlotSortingOrder(ProjectData,[SelectedResponses(2) SelectedResponses(1)],AM_Param,AM_Speed,Units_Sel,Diff);
        % I = descending order taking into account:[pert response; 2s vis stim respo; post-pert reponse]
        I = I(~isnan(B(:,1)));
        B = reshape(B(~isnan(B(:))),length(B(~isnan(B(:))))/3,3);
...
if Diff == 0
    h1 = shadedErrorBar(TimeLine(1:size(SelectedResponses{1},2)),...
        mean(SelectedResponses{1}(~isnan(sum(SelectedResponses{1},2)),:),1),...
    ...
    h2 = shadedErrorBar(TimeLine(1:size(SelectedResponses{2},2)),...
        mean(SelectedResponses{2}(~isnan(sum(SelectedResponses{2},2)),:),1),...
    ...
    Difference = abs(((mean(SelectedResponses{1}(~isnan(sum(SelectedResponses{1},2)),:),1))...
                    -(mean(SelectedResponses{2}(~isnan(sum(SelectedResponses{2},2)),:),1))));
...
if Diff == 0
    if Sep==0
        ll = legend({'running', 'stationary'},'fontsize',10);
        ll.Color = 'none'; ll.EdgeColor = 'none';
    else
        ll = legend({'running', 'stationary'},'fontsize',10);
        ll.Color = 'none'; ll.EdgeColor = 'none';
  ...
  %  ylim([YtraceMax-0.3 YtraceMax+0.1])
    ...
    ll = legend('abs(Run-stationary)','fontsize',10);
    ll.Color = 'none'; ll.EdgeColor = 'none';
...
function imagescSetting(TimeLine)
