...
function PSTH_basic_pert_angle(i,SpikesTrial,ES,FileName,RecordingOI,Direction)
    ...
    plot(linspace(-0.5,(ES.ACInfo.trialStartsEnds(j,2)+0.5)-(ES.ACInfo.trialStartsEnds(j,1)-0.5),size(SpeedTrial,2)),...
         mean(SpeedTrial(find(ES.PertTrial==0 & ES.OrienTrial==Direction),:),1),...
         'Color',[0.6 0.6 0.6],'LineWidth',1.5)
    hold on % plot speed w perturbation
    plot(linspace(-0.5,(ES.ACInfo.trialStartsEnds(j,2)+0.5)-(ES.ACInfo.trialStartsEnds(j,1)-0.5),size(SpeedTrial,2)),...
         mean(SpeedTrial(find(ES.PertTrial==1 & ES.OrienTrial==Direction),:),1),...
    ...
    plot(linspace(-0.5,(ES.ACInfo.trialStartsEnds(j,2)+0.5)-(ES.ACInfo.trialStartsEnds(j,1)-0.5),size(SpikeCount,2)), ...
        smooth(mean(SpikeCount(find(ES.PertTrial==0 & ES.OrienTrial==Direction),:),1)*60,9),'Color',[0.6 0.6 0.6],'LineWidth',2)
    hold on
    plot(linspace(-0.5,(ES.ACInfo.trialStartsEnds(j,2)+0.5)-(ES.ACInfo.trialStartsEnds(j,1)-0.5),size(SpikeCount,2)), ...
        smooth(mean(SpikeCount(find(ES.PertTrial==1 & ES.OrienTrial==Direction),:),1)*60,9),'Color',[0 0 0],'LineWidth',2)
    xlim([-0.5 ES.ACInfo.trialStartsEnds(j,2)-ES.ACInfo.trialStartsEnds(j,1)+0.5])
    ...
    DetailCell = {[FileName{1}(StartLetter+length(pattern)+1:StartLetter+length(pattern)+13) ', ' FileName{1}(end-22:end-4)] , ...
                   ...
                   ', ID=', num2str(ES.SpikeInfo{5,i+1}), ', quality=', ES.SpikeInfo{2,i+1}, ', Direction=', num2str(Direction)))};
    a = axes;
    t1 = title(DetailCell);
    ...
    if exist([FileName{1}(1:StartLetter-1) 'Figures/'], 'dir')
        saveas(gcf,[FileName{1}(1:StartLetter-1) 'Figures' filesep ...
                    'PSTH_Recording_' num2str(RecordingOI) '_cellID_' num2str(ES.SpikeInfo{5,i+1}) '_pert_' num2str(Direction)],'png')
    else
