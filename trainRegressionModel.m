function [trainedModel, validationRMSE] = trainRegressionModel(trainingData,predictorsTBI)
...
% Auto-generated by MATLAB on 21-Jun-2021 10:45:32
...
switch predictorsTBI
    case 1
        inputTable = array2table(trainingData(:,2:4), 'VariableNames', {'Direction', 'MeanSpeed', 'FR_m'});
        predictorNames = {'Direction', 'MeanSpeed'};
        predictors = inputTable(:, predictorNames);
        response = inputTable.FR_m;
        isCategoricalPredictor = [false, false];
    case 2
        inputTable = array2table(trainingData(:,[1 3 4]), 'VariableNames', {'TF', 'MeanSpeed', 'FR_m'});
        predictorNames = {'TF', 'MeanSpeed'};
        predictors = inputTable(:, predictorNames);
        response = inputTable.FR_m;
        isCategoricalPredictor = [false, false];
    case 3
        inputTable = array2table(trainingData(:,[1 2 4]), 'VariableNames', {'TF', 'Direction', 'FR_m'});
        predictorNames = {'TF', 'Direction'};
        predictors = inputTable(:, predictorNames);
        response = inputTable.FR_m;
        isCategoricalPredictor = [false, false];
    case 4
        inputTable = array2table(trainingData, 'VariableNames', {'TF', 'Direction', 'MeanSpeed', 'FR_m'});
        predictorNames = {'TF', 'Direction', 'MeanSpeed'};
        predictors = inputTable(:, predictorNames);
        response = inputTable.FR_m;
...
concatenatedPredictorsAndResponse.column_4 = response;
linearModel = fitlm(...
    concatenatedPredictorsAndResponse, ... %'interactions', ...
    'RobustOpts', 'off');
...
for fold = 1:KFolds
    ...
    concatenatedPredictorsAndResponse.column_4 = trainingResponse;
    linearModel = fitlm(...
        concatenatedPredictorsAndResponse, ... % 'interactions', ...
        'RobustOpts', 'off');
