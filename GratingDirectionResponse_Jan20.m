...
for UI = 1:length(Unit_Inds)
    % for plotting all units
    % UI_idx = Unit_Inds(UI);
    %%%%%%%%%%%%%%%
    % for plotting example units
%     UI_idx = 1;
%     while UI_idx<=830
%         Target(UI_idx) = find(I==UI_idx) == 224;
%         UI_idx = UI_idx + 1;
%     end
%     UI_idx = find(Target)
    %%%%%%%%%%%%%%%%%%%
...
%         end
        ...
        suptitle(['Unit ' num2str(find(UI_idx),3) ', ' ... % num2str(find(I==UI_idx),3)
            'AUC=' num2str(AUC_shuffled(UI_idx,1),2) ', ' ...
            'MI=' num2str(DM(UI_idx),2) ',' ...  %num2str(DM_sh(UI_idx,1),2)
            'Pert.Resp. = ' num2str(PertRespUnits(UI_idx))  ', ' ... num2str(Resp_PERT(UI_idx))
...
for UI = 1:length(Unit_Inds)
    ...
    %if DM(UI_idx)<0
        ...
        Trial_Pert = squeeze(param_sel(:,UI_idx,3));
        figure
        set(gcf,'Position',[-1920 50 1900 200])
        % GratingDirectionsOrdered = [135 90 45 180 0 225 270 315];
        for i = 1 : length(GratingDirections)
            subplot(1,9,i)
            % visual
            % plot all trials
            Trial2plot = find(Trial_Angle==GratingDirections(i));
            %     for j = 1:length(Trial2plot)
            %         plot(TimeLine(1:FirstPart-90),UnitResponse(Trial2plot(j),TimeLine-Pert_lims(1)),'Color',[0.5 0.5 0.5]);
            %         hold on
            %     end
            ...
            set(gca,'box','off','TickDir','out')
            % perturbation
            Trial2plot_p = find(Trial_Angle==GratingDirections(i) & Trial_Pert==1);
            Trial2plot_np= find(Trial_Angle==GratingDirections(i) & Trial_Pert==0);
            % plot all trials
            %     for j = 1:length(Trial2plot)
            %         plot(TimeLine(FirstPart-60:end),UnitResponse(Trial2plot(j),FirstPart-60:end),'Color',[0.5 0.5 0.5]);
            %         hold on
        ...
        % polarplot([deg2rad(GratingDirections); 0],[GratingDirResp(UI_idx,:) GratingDirResp(UI_idx,1)],'r')
        hold on
        plot([(GratingDirections)],[GratingDirResp(UI_idx,:)+GratingDirResp_BL(UI_idx,:) ],'r')
...
%% 
figure
p_thres = 0.01; PertResp_units = (AUC_shuffled(:,1)>p_pert_th);
