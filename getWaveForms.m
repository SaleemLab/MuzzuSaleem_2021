...
% flatten the raw trace (high pass at 300Hz)
fs = 30000;
Channel = single(mmf.Data.x)';
Channel_f = highpass(Channel,300,fs);
Channel_f = Channel_f';
...
for curUnitInd=1:numUnits
    ...
    for curSpikeTime = 1:min([gwfparams.nWf curUnitnSpikes])
        tmpWf = Channel_f(1:gwfparams.nCh,spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(1):spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(end));
